"""Centrifuge control panel for the Opentrons OT-2.

Upload this ONE file to the Opentrons App. Every control the browser console has
is a runtime parameter here: pick an Action in the app's protocol setup screen,
set Force / Time if you're spinning, and press Run.
...
client finds it by handshake, so it can never grab the robot's own controller.

The pipetting steps around a spin live in YOUR protocol -- add them once the
centrifuge has a custom labware definition and a Labware Position Check. This
file deliberately loads no labware and moves no liquid.
"""

metadata = {
    "protocolName": "Centrifuge control panel",
    "description": "Door, tube indexing, spin (RCF + time), and gantry lock debug controls.",
    "author": "Tripathi Lab",
}

# Runtime parameters need API level 2.18 (OT-2 robot software 7.3.0 or newer).
requirements = {"robotType": "OT-2", "apiLevel": "2.18"}


# --- BEGIN INLINE centrifuge.py (generated by tools/sync_inline.py) ---
...
ACTIONS = [
    {"display_name": "Spin (Force + Time)", "value": "spin"},
    {"display_name": "Open door", "value": "door_open"},
    {"display_name": "Close door", "value": "door_close"},
...
def add_parameters(parameters):
    parameters.add_str(
        ...
        choices=ACTIONS,
        default="status",
        description="What the centrifuge should do. Force/Time apply to Spin; Tube applies to Go to tube.",
    )
    parameters.add_int(
        display_name="Force",
        variable_name="rcf",
        ...
        maximum=MAX_RCF,
        unit="x g",
        description="Relative centrifugal force at the tube radius (%.1f mm)." % ROTOR_RADIUS_MM,
    )
    parameters.add_float(
        ...
        maximum=240.0,
        unit="min",
        description="How long to hold at speed, once the ramp-up finishes.",
    )
    parameters.add_int(
        display_name="Ramp",
        variable_name="ramp_seconds",
        default=120,
        minimum=1,
        maximum=600,
        unit="s",
    ...
    parameters.add_int(
        ...
        minimum=1,
        maximum=TUBE_COUNT,
        description="Which tube position to present to the pipette (Go to tube).",
    )
...
def run(protocol):
    ...
    protocol.comment("Centrifuge action: %s" % _label(action))
    if action == "spin":
        protocol.comment(
            "  %d x g (%d rpm) for %g min, %d s ramps"
    ...
    if protocol.is_simulating():
        ...
        # port must not be touched. The run log still shows what would happen.
        protocol.comment("Simulating: no serial connection is opened.")
        return

    # The door and the rotor move on their own; get the pipette out of the way first.
    if action in ("spin", "door_open", "door_close", "rotate"):
        protocol.home()

    centrifuge = Centrifuge()
    protocol.comment("Connected on %s" % centrifuge.port)
    try:
        protocol.comment("Before: %s" % centrifuge.describe())

        if action == "spin":
            _spin(protocol, centrifuge, params)
        elif action == "door_open":
            centrifuge.door_open()
        elif action == "door_close":
            centrifuge.door_close()
        elif action == "rotate":
            centrifuge.rotate(params.tube)
        elif action == "lock":
            centrifuge.lock()
        elif action == "unlock":
            centrifuge.unlock()
            ...
            raise CentrifugeError("unknown action %r" % action)

        protocol.comment("After:  %s" % centrifuge.describe())
    finally:
        centrifuge.close()


def _spin(protocol, centrifuge, params):
    """Run the cycle, reporting progress into the run log every 30 s."""
    last = [0.0]
    ...
    def progress(status):
        ...
        if now - last[0] < 30.0:
            return
        last[0] = now
        protocol.comment("  %s | %d rpm (%d x g)" % (
            STATE_NAMES.get(status.get("STATE"), status.get("STATE")),
            status.get("RPM1", 0),
            round(rpm_to_rcf(status.get("RPM1", 0)))))

    rpm = centrifuge.spin(
        rcf=params.rcf,
        minutes=params.minutes,
        ramp_seconds=params.ramp_seconds,
        on_poll=progress,
    )
    protocol.comment("Spin complete (%d rpm commanded); door is open." % rpm)
